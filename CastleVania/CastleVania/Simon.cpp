...
Simon::Simon(int _posX, int _posY): DynamicObject(_posX, _posY, 0, -SPEED_Y, EnumID::Simon_ID)
{
	...
	_simonKnockBack = new CSprite(Singleton::getInstance()->getTexture(EnumID::Simon_ID), 8, 8, 100);
	...
	else
	{
		...
		else
		{
			...
			if(_hasKnockBack)
			{
				if(_vLast > 0)
				{
					_simonKnockBack->Draw(center.x, center.y);
					return;
				}
				else if(_vLast < 0)
				{
					_simonKnockBack->DrawFlipX(center.x, center.y);
					return;
				}
			}
			else
